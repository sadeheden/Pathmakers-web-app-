...
const CartModal = ({ visible, purchasedItems, onClose }) => {
  ...
  const calculateExpiryDate = (purchaseDate) => {
    if (!purchaseDate) return 'Unknown expiry';
    try {
      const purchase = new Date(purchaseDate);
      const expiry = new Date(purchase);
      expiry.setMonth(expiry.getMonth() + 1); // Add one month
  ...
  // Render single item - no delete button
        ...
        <View style={styles.attractionContent}>
          ...
          <View style={styles.expirationNotice}>
            ...
            <Text style={styles.expirationText}>
              Expires one month after purchase ({expiryDate})
            </Text>
          </View>
