...
class Orders2Controller {
  static async createOrder(req, res) {
    try {
      ...
      // **CRITICAL: Check for existing orders with overlapping dates**
      console.log('🔍 Checking for date conflicts...');
      const destinationToCheck = destinationCityName || destination || cityName;
      
      const conflictingOrder = await orders2DB.findOverlappingOrder({
        userId: req.user.id,
        destinationName: destinationToCheck,
      ...
      if (conflictingOrder) {
        console.log('⚠️ Order creation blocked due to date conflict:', conflictingOrder._id);
        return res.status(409).json({
          success: false,
          message: `You already have a trip to ${destinationToCheck} during these dates`,
          conflict: true,
          existingOrder: {
      ...
      let finalAttractionNames = Array.isArray(attraction_names) ? attraction_names :
        Array.isArray(attractionNames) ? attractionNames : [];
      finalAttractionNames = [...new Set(finalAttractionNames.map(String).filter(Boolean))];

      let finalAttractionIds = Array.isArray(attractions) ? 
        attractions.filter(looksLikeId).map((id) => new ObjectId(id)) : [];

      // Build order data with proper Date objects
      const orderData = {
        ...
        // **IMPORTANT: Store as proper Date objects**
        tripDate: tripDateObj,
        returnDate: returnDateObj,
